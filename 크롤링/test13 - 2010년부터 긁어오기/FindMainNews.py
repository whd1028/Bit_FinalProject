...
class FindMainNews:
    def __init__(self, title, content, time, link, pic_link, media, cat):
        self.title = title[0]
                    ...
                    if atag.has_attr('download') == False:
                        try:
                            ...
                            if link.startswith('https://news.naver.com/main/read.naver?mode=LS2D&mid=shm&sid1=101') or link.startswith('http://news.naver.com/main/read.naver?mode=LS2D&mid=shm&sid1=101'):
                                links.append(link)
                                try:
                                    news = FindMainNews.Extract(link)
    ...
    def Extract(url):
        try :
            ...
            try:
                ...
                return FindMainNews(title, content, time, link, pic_link, media, cat)
