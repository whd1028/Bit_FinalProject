...
class WebPage:
    # 생성자 생성
    # 객체가 존재하는 경우 객체가 생성될 때 해야할 일을 생성자로 정의
    # 객체 메소드와 정적 메소드가 존재한다.
    # 어지간하면 객체 메소드의 처음 인자로 self가 들어간다
    # self가 가장 먼저 들어오지 않으면 정적 메소드
    # 수집한 주소, 이름, 상세내용, 하이퍼링크를 인자로 받는다
        ...
        for atag in atags:
            ...
            if atag.has_attr('download') == False:
                try:
                    ...
                    if str.startswith(link,'http://ehpub.co.kr/') or str.startswith(link,'http://ehpub.co.kr/'): # https도 http로 시작한다
                       links.append(link)
        return links
